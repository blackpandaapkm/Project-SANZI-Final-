...
if (typeof SpeechRecognition === "undefined") {
  startBtn.remove();
  result.innerHTML =
    "Your browser doesn't support Speech API.Please download latest Chrome version.";
}
...
function process(rawText) {
  ...
  if (text.includes("hey")) {
    ...
    response =
      "i am SANZI the ai bot, developing me by HALUM R&D LIMITED. just say halum the great ";
    toggleBtn();
  ...
  //home auto mation command started
    ...
    const Http = new XMLHttpRequest();
    const url = "http://192.168.0.116/car?sanzi=1";
    Http.open("GET", url);
    Http.send();
    Http.onreadystatechange = (e) => {
      console.log(Http.responseText);
